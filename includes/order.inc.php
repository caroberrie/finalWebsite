<form action='<? echo $_SERVER['PHP_SELF']; ?>' method='POST' name='order_form' id='order_form' enctype='multipart/form-data'>
	...
	$select_producst = "SELECT beautys_id, category, description, size, price, stock_quantity, photo
		FROM beautys
	...
	if(!$exec_select_producst){
		...
		while($one_record = mysqli_fetch_assoc($exec_select_producst)){
			...
			echo "<tr>
				...
				<td><input type='number' name='quantity[$beautys_id][$price]' id='quantity' min='0' max='$max'";
					if(isset($quantity)&&!empty($quantity[$beautys_id][$price])) echo "value='{$quantity[$beautys_id][$price]}'";
				echo "></td></tr>";
		}
		echo "</table>";
	...
	<?php
		$select_address = "SELECT address_1, address_2, city, beauty_states_id, zip from beauty_customers WHERE beauty_customers_id=$beauty_customers_id";
		...
		if(!$exec_select_address){
			...
			$beauty_states_id = $one_record['beauty_states_id'];
			$zip = $one_record['zip'];
		}
		...
		$select_states = "SELECT beauty_states_id, state, abbr from beauty_states";
		...
		if(!$exec_select_states){
			...
			create_drop_down_from_query('State: ', 'beauty_states_id', 'beauty_states_id', $multi_array, ['tabindex'=>'9', 'title'=>'State'], $errors_array);
		}
	...
	<fieldset><legend>Shipping Method</legend>	
		<?php
		$select_carriers_methods = "SELECT beauty_carriers_methods_id, carrier, method, fee from beauty_carriers_methods";
		...
		if(!$exec_select_carriers_methods){
			...
			create_drop_down_from_query('Shipping Method: ', 'beauty_carriers_methods_id', 'beauty_carriers_methods_id', $multi_array, ['title'=>'Shipping Method'], $errors_array);
		}
		?>
	</fieldset>
	<fieldset>
