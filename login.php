...
if(isset($_SESSION['beauty_customers_id']) && isset($_SESSION['full_name'])){
	...
	if(isset($_POST['submitted'])){
		...
		if(count($errors_array) == 0){
			$sel_customer = "SELECT beauty_customers_id, password, concat(first_name,' ', last_name) as full_name from beauty_customers WHERE email = '$email'";
			...
			if(!$exec_sel_customer){
				...
				rollback('There seem to be more than one customer with tbeauty email. Please call our customer care.');
			}else{
				$one_record = mysqli_fetch_assoc($exec_sel_customer);
				$hashed_password = $one_record['password'];
				if(password_verify($password, $hashed_password)){
					//setcookie('beauty_customers_id', $one_record['beauty_customers_id'], time()+3600, '/', '', 0, 1);
					//setcookie('full_name', $one_record['full_name'], time()+3600, '/', '', 0, 1);
					$_SESSION['beauty_customers_id'] = $one_record['beauty_customers_id'];
